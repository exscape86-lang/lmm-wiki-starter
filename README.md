...
이 스타터의 `source → concept → daily → project` 흐름은 시작점입니다. 같은 구조 위에 다음 3단계를 얹으면, vault가 단순 보관함을 넘어 **스스로 수집·정리·승격하는 지식 엔진**으로 자랍니다. 아래는 "이렇게 키울 수 있다"는 방향 안내이고, 실제 자동화 스크립트와 완성 세팅은 Pro에서 제공합니다.

1. **주제(topic) 레이어 얹기** — 흩어진 source/concept 위에 주제별 "목차 노트"와 "답변 노트"를 한 장씩 둡니다. 한 주제에 대해 "내가 가진 자료가 뭐고(목차), 결론이 뭔지(답변)"를 한눈에 봅니다. 처음엔 수동으로 한 주제만 만들어 보세요.
2. **데일리 자동 다이제스트** — 오늘 들어온 노트와 오늘 새로 연결된 노트를 매일 한 장으로 모아줍니다. "오늘 뭐가 늘었나"를 매번 손으로 정리하지 않아도 됩니다.
3. **외부 소스 자동 수집 → 후보 → 사람 승격** — 즐겨 보는 외부 출처에서 새 글을 자동으로 긁어와 `daily`에 "후보"로 쌓고, 사람이 보고 쓸 만한 것만 정식 노트로 승격합니다. 자동화는 "거르기"까지, "승격 결정"은 사람이 합니다.

> 완성된 자동화 세팅(수집 스크립트, 주제 답변 노트 생성, 데일리 다이제스트 파이프라인)은 Pro 버전에서 통째로 제공합니다. 무료 스타터는 이 구조를 직접 손으로 굴려볼 수 있게 설계되어 있습니다.
...
4. **자동화가 만든 건 무조건 "후보"로만 둔다.** 사람이 한 번 보고 `confidence`를 붙이고, 정식 노트로 승격할지 결정한다. 자동화는 모으고 거르는 데까지, 승격은 사람이 한다.
5. API 키, 개인정보, 비공개 로그는 vault에 넣지 않는다.

## 무료 버전과 Pro 버전 아이디어

무료 버전은 구조와 설치법, 그리고 "이렇게 자동화로 키울 수 있다"는 방향까지를 담습니다. Pro 버전은 그 방향을 **바로 돌아가는 실물**로 제공해 시간을 줄여줍니다.

- 완성 Vault ZIP (실제로 굴러가는 상태 그대로)
- 플러그인 세팅 스크린샷/영상
- 실제 사용 예시 노트 20개+
- 외부 소스 자동 수집 → 후보 → 승격 파이프라인 실물
- 데일리 자동 다이제스트 / 주제별 답변 노트 자동 생성 세팅
- 자동화 스크립트/업데이트
- 개인 업무별 템플릿 패키지
